...
fn reserved_in_rust(name: &str) -> bool {
    matches!(
        name,
...
fn main() {
    ...
    // rust codegen
    const NODETYPE_JSON: &str = include_str!("../../src/node-types.json");
    let nodetype_json = serde_json::from_str::<Vec<NodeType>>(NODETYPE_JSON);
    match nodetype_json {
        Ok(_) => {}
        Err(e) => panic!("Parse node-type.json failed: {e}"),
    }

    const GRAMMAR_JSON: &str = include_str!("../../src/grammar.json");
    let grammar_json = serde_json::from_str::<Value>(GRAMMAR_JSON);
    match grammar_json {
        Ok(_) => {}
        Err(e) => panic!("Parse grammar.json failed: {e}"),
    }
...
This file is auto-generated by build.rs, which is not intended for manual editing."#,
    ...
    for node_type in nodetype_json.unwrap() {
        if !node_type.named || reserved_in_rust(&node_type.type_name) {
    ...
    let grammar_rs = Path::new("bindings").join("rust").join("grammar.rs");
    generate_file(grammar_rs, grammar_scope.to_string().as_bytes());
